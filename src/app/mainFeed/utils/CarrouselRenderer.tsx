...
const CarrouselRenderer: React.FC<CarrouselRendererProps> = ({ mediaItems, onImageClick, onCommentIconClick }) => {
    ...
    const CustomDot: React.FC<{ onGoToSlide: (index: number) => void; index: number; active: boolean }> = ({ onGoToSlide, index, active }) => (
        <Box
            ...
            {mediaItems.length > 1 ? (
                <Box mt={4}>
                    <Carousel
                        ...
                        customDot={<CustomDot onGoToSlide={(index: number) => carouselRef.current.goToSlide(index)} index={0} active={false} />}
                    >

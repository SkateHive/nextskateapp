...
function PostContent({ user, postId }: { user: string, postId: string }) {
    ...
    const { comments, isLoading, error, updateComments } = useComments(user.substring(3), postId);
    ...
    const getTotalPayout = (post: PostModel) => {
        ...
        const totalPayout = parsePayoutValue(typeof post.total_payout_value === 'string' ? post.total_payout_value : post.total_payout_value.toString());
        const pendingPayout = parsePayoutValue(typeof post.pending_payout_value === 'string' ? post.pending_payout_value : post.pending_payout_value.toString());
        const curatorPayout = parsePayoutValue(typeof post.curator_payout_value === 'string' ? post.curator_payout_value : post.curator_payout_value.toString());

  ...
  return (
    ...
    >
        ...
        {paginatedNfts.map((nft, index) => (
          ...
          >
            {nft.token.medias.length > 0 ? (
              ...
              <Image
                src={`/loading.gif?w=${IMAGE_SIZES.LOADING_MEDIUM.width}&h=${IMAGE_SIZES.LOADING_MEDIUM.height}`}
                alt="Loading"
                boxSize="100%"
                objectFit="cover"
                borderRadius="10px"
      ...
      <HStack justifyContent="space-between" w="100%">
        <Text cursor="pointer" onClick={handlePreviousPage} color="yellow.400">
          Previous
        </Text>
        <Text color="white">
          {currentPage} / {totalPages}
        </Text>
        <Text cursor="pointer" onClick={handleNextPage} color="yellow.400">
          Next
        </Text>
      </HStack>
    </CollapsibleBox>

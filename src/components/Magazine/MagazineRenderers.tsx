...
const ImageRenderer = memo(({ alt, src, title, ...props }: RendererProps) => {
  const processedSrc = useMemo(
    () =>
      src && typeof src === "string"
        ? src.replace("ipfs.skatehive.app", PINATA_URL)
        : "",
    [src]
  );
...
// Memoized paragraph renderer
const ParagraphRenderer = memo(({ children, ...props }: RendererProps) => (
  <div
    {...props}
    style={{ color: "white", fontSize: "10px", paddingBottom: "5px" }}
  >
    {children}
  </div>
));
...
// Memoized anchor renderer
const AnchorRenderer = memo(({ href, children, ...props }: RendererProps) => (
  <a
...
export const MagazineRenderers = {
  ...
  iframe: ({ src, ...props }: RendererProps) => {
    ...
    if (
      zoraRegex.test(String(src)) ||
      threeSpeakRegex.test(String(src)) ||
      youtubeRegex.test(String(src))
    ) {
      return (
        <center>
          <iframe

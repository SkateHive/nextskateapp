...
const MainInput = forwardRef<HTMLTextAreaElement, MainInputProps>(({
            ...
            for (const file of acceptedFiles) {
                ...
                if (fileType.startsWith("video/")) {
                    ...
                    if (ipfsData !== undefined) {
                        ...
                        newMediaList.push({ url: ipfsUrl, type: "video", extension: fileExtension });
                    }
    ...
    const handlePost = async () => {
        ...
        const mediaMarkdown = mediaList.map((media) => {
            if (media.type === "video") {
        ...
        const postData = {
            ...
            parent_permlink: process.env.NEXT_PUBLIC_MAINFEED_PERMLINK || "test-advance-mode-post",
            author: username,
            permlink: permlink,
            title: "Cast",
            body: finalComment,
        ...
        <VStack spacing={3} w="full" p={3} borderRadius="md" {...getRootProps()}>
            <Flex w="full" align="center">
                <Textarea
                    ref={ref}
                    value={comment}
                    onChange={(e) => { setComment(e.target.value); onCommentChange?.(e.target.value); }}
            ...
            <HStack w="full" justify="space-between">
                <HStack spacing={2}>
                    <Tooltip label={isUploadingMedia ? "Uploading Media..." : "Upload Media"}>
                        <IconButton
            ...
            {isPickingEmoji && <EmojiPickerComponent isPickingEmoji={isPickingEmoji} setIsPickingEmoji={setIsPickingEmoji} setComment={setComment} />}
        </VStack>
    );
});
